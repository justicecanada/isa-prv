...
namespace Interview.UI.Services.Stats
{
    ...
    public class StatsManager : IStats
    {
        ...
        public List<VmEquityStat> GetInterviewEquityStats(List<Process> processes, List<Equity> equities, string cultureName)
        {

            List<VmEquityStat> result = new List<VmEquityStat>();
            List<RoleUserEquity> roleUserEquities = processes.SelectMany(x => x.RoleUsers.SelectMany(x => x.RoleUserEquities)).ToList();
            //List<InterviewUser> filteredInterviewUsers = processes.SelectMany(x => x.Interviews.SelectMany(x => x.InterviewUsers
            //    .Where(y => (y.RoleUserType == RoleUserTypes.Interviewer || y.RoleUserType == RoleUserTypes.Lead)))).ToList();
